...
export interface MapdataType {
  name: string;
  value: [number, number, number]; //x,y,value  第一吨x 第二吨y  第三吨value
}
export const optionHandle = (regionCode: string, list: object[], mapData: MapdataType[]) => {
  ...
  return {
    ...
    visualMap: {
      ...
      bottom: 20,
      pieces: [
        { gte: 1000, label: "1000吨以上" }, // 不指定 max，表示 max 为无限大（Infinity）。
        { gte: 600, lte: 999, label: "600-999吨" },
        { gte: 200, lte: 599, label: "200-599吨" },
        { gte: 50, lte: 199, label: "49-199吨" },
        { gte: 10, lte: 49, label: "10-49吨" },
        { lte: 9, label: "1-9吨" }, // 不指定 min，表示 min 为无限大（-Infinity）。
      ],
      inRange: {
        // 渐变颜色，从小到大
